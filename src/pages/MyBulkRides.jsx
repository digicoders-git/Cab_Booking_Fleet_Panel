...
import { getMyBulkRides, assignDriverToBulk, downloadSecurityReceipt } from "../api/bulkBookingApi";
import { assignmentApi } from "../api/assignmentApi";
import { toast } from "sonner";
import {
    FaCar, FaArrowRight, FaTruck, FaPhone, FaUser,
    FaCalendarAlt, FaRoad, FaCheckCircle, FaClock, FaLayerGroup, FaTimes, FaPlusCircle, FaDownload
} from "react-icons/fa";
...
export default function MyBulkRides() {
    ...
    const generateReceipt = async (booking) => {
        try {
            ...
            const blob = await downloadSecurityReceipt(booking._id);
            const url = window.URL.createObjectURL(blob);
            const a = document.createElement("a");
            a.href = url;
            a.download = `KwikCabs_Security_${booking._id.toString().slice(-6).toUpperCase()}.pdf`;
                                ...
                                >
                                    ...
                                    <div className="flex items-center gap-3">
                                        <button
                                            ...
                                            className="px-3 py-1 rounded-lg text-[10px] font-bold flex items-center gap-1.5 transition-all hover:opacity-80"
